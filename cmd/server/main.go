// Command server is the Governance Layer entry point. It starts the REST API
// (port 8083 by default) for frontend/aaa communication, backed by
// PostgreSQL. It is a drop-in replacement for the original Node service
// (src/server.js) — same port, env vars, database and behaviour.
package main
...
func main() {
	...
	<-stop
	log.Println("[INFO] Signal received: closing HTTP server")
